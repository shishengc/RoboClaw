...
```text
从 source tag 的位置抓取目标物体，再放置到 destination tag 的位置上。
```

## 控制流程

脚本不调用高层固定抓取动作，而是依次调用底层原子动作：

```text
1. /skill/detect_tags
2. /skill/get_tag_pose, 查询 source tag
3. /skill/get_tag_pose, 查询 destination tag
4. /skill/gripper, 打开右夹爪
5. /skill/move_eef, 移动到 source tag 抓取预靠近点
6. /skill/move_eef, 移动到 source tag 抓取点
7. /skill/gripper, 关闭右夹爪
8. /skill/move_eef, 抓取后沿 base_link +Z 抬起
9. /skill/move_eef, 移动到 destination tag 放置上方
10. /skill/move_eef, 下降到 destination tag 放置点
...
如果抓取 offset 和放置 offset 不同，传 8 个参数：
...
旧格式仍然兼容：只传 3 个或 6 个 offset 时，脚本会使用环境变量
`SOURCE_TAG_ID` / `DEST_TAG_ID`，默认是 `0 -> 1`。
...
前三个 offset 是 source tag 抓取偏移：

```text
grasp_base = source_tag_base + [grasp_dx, grasp_dy, grasp_dz]
```

后三个 offset 是 destination tag 放置偏移：

```text
place_base = dest_tag_base + [place_dx, place_dy, place_dz]
```
...
- source tag 和 destination tag 都需要在当前相机视野中可检测，脚本会先执行 `detect_tags`。
